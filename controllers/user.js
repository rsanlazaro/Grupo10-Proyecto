...
exports.controller = {
    ...
    viewedit: (req, res) => {
        if (req.session.name) {
            let user = req.session.userID;
            var userIndex = users.findIndex((item) => item.userID == user);
            log.controller.updateSession(req, res);
            console.log(users[userIndex])
            if (user) {
                res.render('../views/users/profile.ejs', {
                    msg: 'Data updated',
    ...
    edit: (req, res) => {
        ...
        upload(req, res, (error) => {
            if (error) {
                ...
                console.log("req.body -> ")
                console.log(req.body)
                errors = validation.controller.checkValid(req.body, req.params.id);
                ...
                if (!validation.controller.isError()) {
                    ...
                    // if(!(validation.controller.isChanged(req.params.id,req.body.email))){
